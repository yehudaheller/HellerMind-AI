...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- HellerMind-AI: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via the HellerMind-AI bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'yehudaheller/HellerMind-AI';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'HellerMind-AI-bug-reporter',
    });
