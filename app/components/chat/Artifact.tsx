...
export const Artifact = memo(({ artifactId }: ArtifactProps) => {
  ...
  const runningFileActions = actions.filter((action) => action.type === 'file' && action.status === 'running');
  ...
  return (
    <>
      <div className="artifact border border-bolt-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
        ...
        {artifact.type === 'bundled' && (
          <div className="bg-bolt-elements-actions-background border-t border-bolt-elements-artifacts-borderColor">
            <div
              className={classNames('flex items-center gap-2 p-5', {
                'hm-shimmer': !allActionFinished,
              })}
            >
            ...
            {!allActionFinished && runningFileActions.length > 0 && (
              <div className="border-t border-bolt-elements-artifacts-borderColor px-5 py-3">
                <ActionList actions={runningFileActions} />
              </div>
            )}
          </div>
        )}
...
function ShellCodeBlock({ classsName, code }: ShellCodeBlockProps) {
  ...
  useEffect(() => {
    ...
    highlightCode(code, 'shell', 'dark-plus')
      .then((html) => {
        if (!cancelled) {
      ...
      .catch(() => {
        // The plain-text fallback below remains available if highlighting fails.
      });
  ...
  return (
    <div className={classNames('text-xs', classsName)}>
      {highlightedCode ? (
        ...
        <pre className="m-0 overflow-x-auto rounded-md bg-bolt-elements-actions-code-background p-3">
          <code>{code}</code>
        </pre>
      )}
    </div>
      ...
      <ul className="list-none space-y-2.5">
        {actions.map((action, index) => {
          ...
          return (
            ...
            >
              <div
                className={classNames('flex items-center gap-1.5 text-sm rounded-md', {
                  'hm-file-create-shimmer hm-shimmer px-2 py-1 -mx-2': type === 'file' && status === 'running',
                })}
              >
